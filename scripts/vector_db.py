...
for index, row in tqdm(df.iterrows(), total=df.shape[0]):
    ...
    for i, paragraph in enumerate(paragraphs):
        ...
        paragraph_data.append([article_name, article_url, i + 1, paragraph_text_cleaned, embedding_str, created_at])

# Convert the paragraph data to a DataFrame
paragraph_df = pd.DataFrame(paragraph_data, columns=['article_name', 'article_url', 'paragraph_number', 'paragraph_text', 'embedding', 'created_at'])
